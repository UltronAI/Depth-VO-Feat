...
def upconv(in_planes, out_planes, kernel_size=4, stride=2, padding=1, output_padding=0):
    return nn.Sequential(
...
class QuantPoseExpNet(nn.Module):

    def __init__(self, nb_ref_imgs=2, output_exp=False):
        super(QuantPoseExpNet, self).__init__()
        self.nb_ref_imgs = nb_ref_imgs
        self.output_exp = output_exp
        ...
        # self.conv1 = conv(3*(1+self.nb_ref_imgs), conv_planes[0], kernel_size=7)
        # self.conv2 = conv(conv_planes[0], conv_planes[1], kernel_size=5)
        self.conv1_1 = nn.Conv2d(3*(1+self.nb_ref_imgs), conv_planes[0], kernel_size=1, stride=1, padding=0)
        self.conv1_2 = conv(conv_planes[0], conv_planes[0])
        self.conv2_1 = nn.Conv2d(conv_planes[0], conv_planes[1], kernel_size=1, stride=1, padding=0)
        self.conv2_2 = conv(conv_planes[1], conv_planes[1])
        self.conv3 = conv(conv_planes[1], conv_planes[2])
        self.conv4 = conv(conv_planes[2], conv_planes[3])
        self.conv5 = conv(conv_planes[3], conv_planes[4])
        self.conv6 = conv(conv_planes[4], conv_planes[5])
        ...
        if self.output_exp:
            upconv_planes = [256, 128, 64, 32, 16]
            self.upconv5_ = upconv(conv_planes[4],   upconv_planes[0], kernel_size=3, output_padding=1)
            self.upconv4_ = upconv(upconv_planes[0], upconv_planes[1], kernel_size=3, output_padding=1)
            self.upconv3_ = upconv(upconv_planes[1], upconv_planes[2], kernel_size=3, output_padding=1)
            self.upconv2_ = upconv(upconv_planes[2], upconv_planes[3], kernel_size=3, output_padding=1)
            self.upconv1_ = upconv(upconv_planes[3], upconv_planes[4], kernel_size=3, output_padding=1)
    ...
    def forward(self, target_image, ref_imgs):
        ...
        if self.output_exp:
            out_upconv5 = self.upconv5_(out_conv5  )[:, :, 0:out_conv4.size(2), 0:out_conv4.size(3)]
            out_upconv4 = self.upconv4_(out_upconv5)[:, :, 0:out_conv3.size(2), 0:out_conv3.size(3)]
            out_upconv3 = self.upconv3_(out_upconv4)[:, :, 0:out_conv2.size(2), 0:out_conv2.size(3)]
            out_upconv2 = self.upconv2_(out_upconv3)[:, :, 0:out_conv1.size(2), 0:out_conv1.size(3)]
            out_upconv1 = self.upconv1_(out_upconv2)[:, :, 0:input.size(2), 0:input.size(3)]
    ...
    def copy_params_from_pretrained(self, pose_exp_net):
        for name, l1 in pose_exp_net.named_children():
            try:
