...
parser.add_argument('--caffe-model', default=None, type=str)
...
def loadWeightsFromCaffe(caffe_model):
    """ load weights from caffe model to pytorch model 
    return state_dict
    """
    weights = {}
    # TODO:
...
def main():
    ...
    # create model
    model = OdometryNet(bit_width=BITWIDTH)
    if args.caffe_model is None and args.checkpoint is None:
        model.init_weights()
    elif args.checkpoint:
        weights = torch.load(args.checkpoint)
        OdometryNet.load_state_dict(weights)
    elif args.caffe_model:
        weights = loadWeightsFromCaffe(args.caffe_model)
        OdometryNet.load_state_dict(weights)
